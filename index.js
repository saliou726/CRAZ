...
console.log("Session downloaded succesfuly ✅")
})})}
...
const port = process.env.PORT || 9090;
  ...
  console.log("Connecting to WhatsApp pls wait...");
  ...
  console.log('🧬 Installing Crazy commands')
  const path = require('path');
  fs.readdirSync("./V3/").forEach((plugin) => {
...
> © Powered by Crazy Dev — CRAZY TECH INC`;
  ...
  if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_REACT === "true"){
    const crazylike = await conn.decodeJid(conn.user.id);
    const emojis = ['❤️', '💸', '😇', '🍂', '💥', '💯', '🔥', '💫', '💎', '💗', '🤍', '🖤', '👀', '🙌', '🙆', '🚩', '🥰', '💐', '😎', '🤎', '✅', '🫀', '🧡', '😁', '😄', '🌸', '🕊️', '🌷', '⛅', '🌟', '🗿', '🇵🇰', '💜', '💙', '🌝', '🖤', '💚'];
    ...
    }, { statusJidList: [mek.key.participant, crazylike] });
  }                       
  ...
  const udp = botNumber.split('@')[0];
    const crazy = ('24105730123');
    let isCreator = [udp, crazy, config.DEV]
...
if (senderNumber.includes("221752238497") && !isReact) {
